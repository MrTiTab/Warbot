...
async def _regen_stat(user_id: int, current_field: str, max_field: str, last_update_field: str, regen_minutes: int):
    ...
    if regen_amount > 0:
        new_value = min(maximum, current + regenMAX_ENERGY = 100
ENERGY_REGEN_MINUTES = 5
...
async def get_army(user_id: int) -> dict:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute("SELECT * FROM user_army WHERE user_id = ?", (user_id,))
        rows = await cur.fetchall()
...
async def update_military_power(user_id: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET military_power = ? WHERE user_id = ?", (power, user_id))
        await db.commit()
    return power
...
async def recruit_unit(user_id: int, unit_id: str, quantity: int = 1):
    ...
    if user["coins"] < cost_coins:
        return False, f"سکه کافی نداری! نیاز: {cost_coins} — موجودی: {user['coins']}"
    if user["gold"] < cost_gold:
        return False, f"طلای کافی نداری! نیاز: {cost_gold} — موجودی: {user['gold']}"
...
async def collect_army_upkeep(user_id: int) -> int:
    ...
    last_update_raw = user.get("last_upkeep_update") or user["last_energy_update"]
...
async def get_user_rank(user_id: int) -> int:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT COUNT(*) + 1 FROM users WHERE is_registered = 1 AND military_power > ?",
...
def army_hub_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(text="⚔️ استخدام و ارتش من", callback_data="menu:barracks")
...
def barracks_kb(barracks_level: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for unit_id in UNIT_ORDER:
        unit = UNITS[unit_id]
        if barracks_level < unit["unlock_level"]:
            builder.button(
                text=f"🔒 {unit['name']} (نیاز: پادگان سطح {unit['unlock_level']})",
                callback_data="locked:unit",
            )
            continue
        gold_part = f" + {unit['cost_gold']}🪙" if unit["cost_gold"] else ""
        builder.button(
            text=f"{unit['name']} — {unit['cost_coins']}💰{gold_part} (۱x)",
            callback_data=f"recruit:{unit_id}:1",
        )
        builder.button(
            text=f"{unit['name']} — ۱۰x",
            callback_data=f"recruit:{unit_id}:10",
        )
    builder.button(text="⬅️ بازگشت", callback_data="menu:army")
    builder.adjust(2)
    return builder.as_markup()
...
def barracks_text(user: dict, buildings: dict, army: dict) -> str:
    barracks_level = buildings.get("barracks", {}).get("level", 0)
    capacity = army_capacity(barracks_level)
    ...
    lines = [
        ...
        f"👥 نیرو: {total_units}/{capacity}",
        f"💸 هزینه نگهداری: {hourly_upkeep} سکه/ساعت\n",
        "<b>واحدهای من:</b>",
    ]
    has_units = False
    for unit_id in UNIT_ORDER:
        ...
        if count > 0:
            has_units = True
            lines.append(f"{UNITS[unit_id]['name']}: {count}")
    if not has_units:
        lines.append("هنوز هیچ واحدی نداری.")

    lines.append("\n<b>لیست استخدام:</b> (دکمه‌های پایین رو بزن)")
    return "\n".join(lines)


def leaderboard_text(rows: list, me_rank: int, me_power: int) -> str:
    medals = ["🥇", "🥈", "🥉"]
    lines = ["🏆 <b>رتبه‌بندی جهانی فرمانده‌ها</b>\n"]
    for i, row in enumerate(rows):
        medal = medals[i] if i < 3 else f"{i + 1}."
        country = COUNTRIES.get(row["country"], {}).get("name", "❓")
        lines.append(f"{medal} {row['full_name']} — {country} — لول {row['level']} — 💪 {row['military_power']}")
    lines.append(f"\n📍 رتبه‌ی تو: #{me_rank} — قدرت نظامی: {me_power}💪")
    return "\n".join(lines)
...
async def show_profile(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💰 سکه: {user['coins']}\n"
        f"🪙 طلا: {user['gold']}\n"
        f"💪 قدرت نظامی: {user['military_power'] or 0} (رتبه #{rank})"
    )
...
async def show_army_hub(callback: CallbackQuery):
    await collect_army_upkeep(callback.from_user.id)
    await callback.message.edit_text(
        "🏛 <b>مرکز فرماندهی</b>\n\nاز اینجا کشورت رو بساز و ارتشت رو مدیریت کن:",
...
@router.callback_query(F.data == "menu:barracks")
async def show_barracks(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        barracks_text(user, buildings, army),
        reply_markup=barracks_kb(barracks_level),
    )
    await callback.answer()
